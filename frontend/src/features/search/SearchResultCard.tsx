...
import type { SearchMatch, SearchResult, SearchSource } from "./types";
...
const SOURCE_LABELS: Record<SearchSource, string> = {
  ...
  note_title: "笔记标题", note_content: "笔记正文", research_background: "研究背景",
  research_problem: "研究问题", research_method: "研究方法", research_contribution: "创新点",
  research_experiment: "实验", research_conclusion: "研究结论", research_thought: "我的思考",
};
...
export function SearchResultCard({ result }: { result: SearchResult }) {
  if (result.entity_type === "note") {
    const structured = result.matches.some((match) => match.source.startsWith("research_"));
    const target = `/notes/${result.note.id}${structured ? "?view=structured" : ""}`;
    return <article className="card transition-shadow hover:shadow-md">
      <Link to={target} className="flex items-start gap-3"><NotebookPen className="mt-0.5 h-4 w-4 shrink-0 text-primary-500" /><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="rounded bg-primary-50 px-1.5 py-0.5 text-[10px] font-medium text-primary-700">笔记</span><h2 className="text-sm font-medium text-gray-900 dark:text-gray-100">{result.note.title}</h2></div>{result.note.paper_title && <p className="mt-1 truncate text-xs text-gray-500">属于：{result.note.paper_title}</p>}</div><span className="shrink-0 text-[11px] text-gray-400">命中 {result.match_count} 处</span></Link>
      <div className="mt-3 space-y-2">{result.matches.slice(0, 3).map((match, index) => <Link key={`${match.source}-${index}`} to={target} className="block rounded-md bg-gray-50 px-3 py-2 hover:bg-gray-100 dark:bg-slate-800 dark:hover:bg-slate-700"><span className="rounded bg-white px-1.5 py-0.5 text-[10px] font-medium text-primary-700 dark:bg-slate-900 dark:text-primary-200">{SOURCE_LABELS[match.source]}</span><p className="mt-1 line-clamp-2 text-xs leading-5 text-gray-600 dark:text-gray-300">{match.snippet || match.text}</p></Link>)}</div>
    </article>;
  }
