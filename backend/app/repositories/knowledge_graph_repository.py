...
class KnowledgeGraphRepository:
    ...
    async def workspace_node_selection(self, user_id, limit_nodes):
        ...
        degrees = select(
            endpoints.c.paper_id, func.count().label("degree")
        ).group_by(endpoints.c.paper_id).subquery()
        ...
        selected = await self.db.scalars(
            ...
            .order_by(degrees.c.degree.desc(), Paper.updated_at.desc(), Paper.id)
            .limit(limit_nodes)
        )
